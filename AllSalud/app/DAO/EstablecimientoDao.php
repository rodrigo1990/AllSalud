...
class EstablecimientoDao
{
    ...
    public function updateEstablecimiento($request){
      ...
      foreach ($request->establecimientos as $domicilio) {
            ...
            $domicilioUpdate = Domicilio::find($domicilio['establecimiento_ciudad_id']);

            if($domicilioUpdate){//si el domicilio era existente lo busco por su id y adjunto los tipos

              foreach($domicilio['tipos'] as $tipo){
                
                if($tipo!="null")
                $domicilioUpdate->Tipo()->attach($tipo);

              }

            
            }else{//si no era existente busco el ultimo id insertado y adjuto los tipos

              $domicilioUpdate=Domicilio::orderBy('id', 'desc')->first();
      ...
      $k=0;
      if($request->tiposExistentes){
        foreach($request->tiposExistentes as $tipoExistente ){
          $k++;
          if($tipoExistente['tipo'][$k]=="eliminar"){

            DB::table('tipo_establecimiento_domicilio')
            ->where('id',$tipoExistente['registro_id'][$k])
            ->delete();
          }
        }
      }  
